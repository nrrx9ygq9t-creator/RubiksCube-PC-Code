...
# 透视变换的四个点
pts1 = np.float32([[259, 35], [423, 248], [79, 224], [238, 432]])  # 第一张图
...
def grb_color_fo():
    # 读取图像
    img = cv2.imread('./img/raw/codecam_3.png')
    if img is None:
        print("无法读取图像，请检查路径：./img/raw/codecam_3.png")
        return
    ...
    # 保存变换结果（可选）
    cv2.imwrite("./img/aft/foOutput1.png", imgOutput1)

    # 识别两个面的颜色
    face2_colors = analyze_face(imgOutput1)

    # 存储到外部模块
    vision_params.face_col2 = face2_colors

    # 打印结果
    print("第三面颜色（存入 vision_params.face_col2）：")
    for row in face2_colors:
        print(row)
