...
# 透视变换的四个点
pts1 = np.float32([[416, 0], [459, 205], [176, 0], [143, 211]])  # 第一张图
pts2 = np.float32([[460, 207], [413, 393], [143, 213], [180, 390]])  # 第二张图
...
def grb_color_do():
    ...
    img = cv2.imread('./img/raw/codecam_4.png')
    if img is None:
        print("无法读取图像，请检查路径：./img/raw/codecam_4.png")
        return
    ...
    # 保存变换结果（可选）
    cv2.imwrite("./img/aft/doOutput1.png", imgOutput1)
    cv2.imwrite("./img/aft/doOutput2.png", imgOutput2)

    # 识别两个面的颜色
    face0_colors = analyze_face(imgOutput1)
    face1_colors = analyze_face(imgOutput2)

    # 存储到外部模块
    vision_params.face_col0 = face0_colors
    vision_params.face_col1 = face1_colors

    # 打印结果
    print("第一面颜色（存入 vision_params.face_col0）：")
    for row in face0_colors:
        print(row)

    print("\n第二面颜色（存入 vision_params.face_col1）：")
    for row in face1_colors:
        print(row)
